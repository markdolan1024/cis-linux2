		...
		var dataString = 'UserLoged='+UserLoged+'&DisplayStreamBySpecifiUser=true';
			...
			$.ajax({
				  ...
				  success: function(data) {
                     ...
                     '<div class="lineMembersTeam" onclick="selectStreamByLine('+Stream_Id[i]+',\''+Stream_Name[i]+'\',\''+Water_body[i]+'\','+Lat[i]+','+Lon[i]+',\''+DateStream[i]+'\',\''+Time[i]+'\','+Leader_Id[i]+','+idTeam[i]+',\''+NameTeam[i]+'\',\''+Description[i]+'\',\''+URL[i]+'\',\''+TeamImg[i]+'\',\''+Remarks[i]+'\','+UserLoged+')">'+
...
displayPhotos = function(Observations_Id){
	...
	if (data.Status == 'success'){
		...
		$('#photoDisplay').after("<input name=\"download\" onclick=\"downloadPhotos()\" type=\"button\" value=\"Download\"/>");
	}else{
